/**
 * Test script for TMDB movie search functionality
 *
 * Tests:
 * 1. Searching for 'Inception' returns correct results
 * 2. Pagination returns different results for page 2
 */

import { searchMovies } from "../lib/tmdb/client.ts";

/**
 * Test searching for 'Inception'
 */
async function testSearchInception() {
  ...
  try {
    ...
    if (results.results.length === 0) {
      console.error("✗ No results returned for 'Inception' search");
      return false;
    }
    ...
    if (
      !firstResult.tmdb_id ||
      firstResult.type !== "movie" ||
      !firstResult.title
    ) {
      console.error("✗ Result structure is invalid");
      console.error(`  Expected: { tmdb_id, type: 'movie', title, ... }`);
...
async function testPagination() {
  ...
  try {
    const page1 = await searchMovies("movie", 1);
    const page2 = await searchMovies("movie", 2);
    ...
    const page1Ids = new Set(page1.results.map((m) => m.tmdb_id));
    const page2Ids = new Set(page2.results.map((m) => m.tmdb_id));
...
async function testInvalidQuery() {
  console.log("\nTest 3: Error handling for invalid query");
...
async function testInvalidPage() {
  console.log("\nTest 4: Error handling for invalid page number");
...
async function main() {
  ...
  console.log("TMDB Movie Search Test Suite");
  console.log("=".repeat(60));
  console.log();
